...
**Version:** v1.2 (Steelman complete)
**Stand:** 2026-02-27
**Methode:** 6 parallele Such-Agenten, 327 Docs durchsucht, dreistufiges Gap-Modell + Steelman-Review (11/11 Korrekturen)
...
| 2 Packaging | 11 | 4 | 6 | 1 | 4 | 5 | 2 | 0 |
| 3 Entities + DIDs | 13 | 1 | 8 | 4 | 1 | 5 | 5 | 2 |
| 4 Dokumentation | 6 | 0 | 6 | 0 | 0 | 6 | 0 | 0 |
| **Gesamt** | **59** | **34** | **20** | **5** | **18** | **23** | **12** | **6** |
...
**Steelman-Hinweis:** Die FIT-Klassifizierung fuer Phase 1 ist teils tautologisch: implementierte Features haben per Definition lokale Evidenz (der Code selbst ist die Evidenz). Die Gap-Analyse dokumentiert hier primaer die Qualitaet der Wissensbasis VOR Implementation — relevant fuer die Einschaetzung aehnlicher kuenftiger Features, nicht als Qualitaetsaussage ueber den bestehenden Code.
...
| PKG-07 | Config Schema-Version | PARTIAL-EXTERNAL | 🟡 | datapoints.yaml, PROJECT_ASSESSMENT.md | Kein Versioning-Pattern lokal, aber Schema-Versioning ist Standard-Practice (SemVer, OpenAPI, JSON Schema). **Steelman:** Kein Research noetig — dies ist eine Governance-Entscheidung (welches Format, wann inkrementieren), kein Wissensgap | Entscheidung: version-Feld in YAML Header + Migration-Policy definieren. Pattern von etablierten Tools (docker-compose, k8s) uebernehmen | MEDIUM |
...
**Phase 2 Zusammenfassung:** 4 FIT, 5 PARTIAL-LOCAL, 2 PARTIAL-EXTERNAL, 0 GAP-RESEARCH. 1 Blocker (PKG-02: package-data fehlt). **Steelman-Korrekturen:** PKG-02 von FIT→PARTIAL-LOCAL herabgestuft (🔴) — pyproject.toml fehlt `package-data` Config. PKG-07 von GAP-RESEARCH→PARTIAL-EXTERNAL (🟡) — Schema-Versioning ist Governance-Entscheidung, kein Wissensgap.
...
| ENT-02 | Button Discovery | PARTIAL-EXTERNAL | 🟡 | discussion #318 | DHW-Ladung als stateless Aktion erwaehnt. Kein HA button MQTT Config in lokalen Docs. **Steelman:** button ist trivialstes HA Entity (nur command_topic + payload_press) — kein Research, nur Docs-Lookup | HA MQTT Discovery Docs lesen; 5min Aufwand. Kein echter Blocker trotz fehlender lokaler Evidenz | MEDIUM |
| ENT-03 | Water Heater Discovery | PARTIAL-LOCAL | 🔴 | wiki/090:158-200, discussion #54 | Wiki-Beispiel ist fuer **Vitodens 300-W** (Gaskessel), NICHT Vitocal 250-A. DHW-Modi unterschiedlich (gas vs heat_pump). Discussion #54 hat Vitocal-Ansatz aber `mode_command` ist AUSKOMMENTIERT | DID→Mode Mapping fuer Vitocal 250-A recherchieren; DID 531 Write-Verhalten testen | HIGH |
...
| DATA-02 | COP-Berechnung Templates | PARTIAL-LOCAL | 🔴 | discussion #161, VITOCAL_250A_DOCUMENTATION.md | Energie-DIDs dokumentiert, ABER COP fundamental biased: Viessmann zaehlt Abtau-Zyklen als positive Leistung (discussion #161). Kein Community-Workaround bekannt | Experiment noetig: Abtau-Phasen erkennen (DID 2335?) und aus COP-Berechnung ausschliessen. Ohne Loesung nur COP-Warnung in Docs | HIGH |
...
**Phase 3 Zusammenfassung:** 1 FIT, 5 PARTIAL-LOCAL, 5 PARTIAL-EXTERNAL, 2 GAP-RESEARCH. 4 Blocker (DATA-01, DATA-02, SAFE-02, ENT-03). Staerkstes Gebiet: DID-Daten (DATA-03 FIT, DATA-05/06 PARTIAL-LOCAL). Schwachstellen: Write-Safety (SAFE-02 Upstream-Bug) und water_heater DHW-Modi (ENT-03 Wiki-Beispiel ist Vitodens, nicht Vitocal). **Steelman-Korrekturen:** ENT-02 von GAP-RESEARCH→PARTIAL-EXTERNAL (button ist trivialstes HA Entity); ENT-03 von 🟢→🔴 (Wiki-Beispiel passt nicht zu Vitocal 250-A); DATA-02 von 🟡→🔴 (Abtau-Bias macht COP fundamental ungenau).
...
| DOC-05 | CAN-Topologie und Verkabelung | PARTIAL-LOCAL | 🟡 | TECHNICAL_REFERENCE:29-62, discussion #217, wiki/020, discussion #326 | Stecker 91/72 Pinout, Twisted-Pair, GND-Isolation, F.764/F.1034 dokumentiert, aber verstreut. **GND-Widerspruch:** TECHNICAL_REFERENCE sagt GND-Isolation, Viessmann Manual P.231 sagt "kein GND auf internem CAN", wiki/020 sagt "Klemme 2 GND nicht mehr belegt", discussion #217 sagt isolierte Adapter brauchen GND fuer Transceiver. Widerspruch muss im Guide aufgeloest werden (adapter-abhaengig) | Konsolidierter CAN-Wiring-Guide: GND-Entscheidungsbaum (isolierter vs nicht-isolierter Adapter), Adapter-Matrix, Fehlervermeidung | CRITICAL |
...
**Problem:** CAN-Verkabelungswissen existiert verstreut ueber 5+ Quellen (TECHNICAL_REFERENCE, wiki/020, discussions #206/#217/#326). Haeufigste Fehlerquelle laut Community. **GND-Widerspruch:** Quellen widersprechen sich bzgl. GND auf Stecker 91 — Viessmann Manual sagt "kein GND auf internem CAN", aber isolierte CAN-Adapter brauchen GND fuer Transceiver-Referenz (discussion #217). wiki/020 bestaetigt: "Klemme 2 GND nicht mehr belegt" in neuerer Montage-Anleitung.

**Mitigation:**
- Konsolidierter CAN-Guide: Pinout Stecker 91, Twisted-Pair, GND-Entscheidungsbaum, Adapter-Matrix
- GND-Loesung: Adapter-abhaengig (isoliert: GND noetig; nicht-isoliert: GND vermeiden wegen Ground-Loop)
- F.764/F.1034 Fehlervermeidung als Checkliste
- TECHNICAL_REFERENCE:29-62 hat bereits die meisten Daten

**Risiko:** MEDIUM — Schreibaufwand, GND-Widerspruch ist aufloesbar (adapter-abhaengig)
...
| 8 | DID 2580 (CompressorSetpointRps): War O3EInt8, korrekt O3EInt16 scale=10 (Fix in v0.5.x). Falscher Codec liefert abgeschnittene Werte — Bridge muss zum richtigen Codec passen | issue #189, TECHNICAL_REFERENCE:189 | datapoints.yaml Codec-Definition pruefen; bei Firmware-Abhaengigkeit als Variante dokumentieren | MEDIUM |
| 9 | DID 881 (MixerTwoCircuitCentralHeatingCurve): Sub-type Write problematisch — Complex-DID mit Gradient/Level/BasePoint erfordert vollstaendiges JSON mit korrekter Feld-Reihenfolge | discussion #210 | Write-Template muss alle Sub-Felder enthalten; Partial-Write-Verhalten testen | MEDIUM |
| 10 | DID 1603 (PointOfCommonCouplingPower): Firmware-abhaengig 4 vs 12 Bytes (discussion #315). Alte FW: 2x Int16 (Active/ReactivePower). Neue FW: 2x Int16 + 2x Int32 (32-bit Varianten). Bridge Config muss Firmware-Variante beruecksichtigen | discussion #315, TECHNICAL_REFERENCE:188 | VX3-spezifisch (ECU 0x6a1); Config-Varianten oder Auto-Detection via Byte-Laenge | HIGH |
...
- [x] New Findings: 10 Eintraege mit Requirement-Vorschlag
- [x] Executive Summary Counts = Tabellen-Counts (34 🟢 + 20 🟡 + 5 🔴 = 59)
- [x] Blocker-Tabelle = 5 Eintraege (1x Phase 2, 4x Phase 3)
- [x] Detail-Tabellen konsistent mit Executive Summary und Blocker-Tabelle
- [x] PROJECT_ASSESSMENT Scores referenziert und konsistent (5 Dimensionen geprueft)
- [x] Steelman v1.1: Alle 11 Korrekturen eingearbeitet (5 kritische + 6 ergaenzende)

---

*Erstellt: 2026-02-27 | v1.2 Steelman complete | Methode: 6 parallele Such-Agenten (MQTT, HA Discovery, DID/Codecs, Config, Packaging, Docs) ueber 327 Quellen*
